...
for k=1:length(allfwhm),
    ...
    for l=1:n_layers
        fprintf('\n Layer %d', l);
        %l_smoothmeshname=[a1 filesep sprintf('FWHM%3.2f_%d_',fwhm,l) b1 '.mat']
        %if ~exist(l_smoothmeshname)
            ...
            %for j = 1:Ns,
                ...
                d = spm_mesh_geodesic(M,(l-1)*Ns_per_layer+j-1,order);
            ...
            %save(l_smoothmeshname,'QG','M','-v7.3');
        %end
    end
...
%     for l=1:n_layers
%         l_smoothmeshname=[a1 filesep sprintf('FWHM%3.2f_%d_',fwhm,l) b1 '.mat'];
%         x=load(l_smoothmeshname);
%         QG((l-1)*Ns_per_layer+1:l*Ns_per_layer,l*Ns_per_layer)=x.QG;
%     end
